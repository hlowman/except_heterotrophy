...
autotrophic = across_sites_model_data %>%
  ...
  select(site_name, year, lat, lon, ann_GPP_C, ann_ER_C, ann_NEP_C,  PR,PAR_sum, Disch_cv, max_interstorm, RBI, width_to_area, PrecipWs, MOD_ann_NPP, drainage_density_connected, PAR_kurt, ElevWs, Width, Stream_PAR_sum, max_NEP) %>%
...
litauto2<-litauto %>% mutate(primary_class = case_when(is.na(primary_class) == T~Class, T~primary_class)) %>% 
                      mutate(secondary_class = case_when(is.na(secondary_class)==T~ "none", T~secondary_class)) %>%
                      mutate(Group = case_when(is.na(Group) == T~Class, T~Group)) %>% 
                      mutate(dam = case_when(dam == 0~"no_dam", 
                                             dam ==1 ~"dam", 
                                             is.na(dam)==T~ "unknown")) 
                
                  # select(site_name,year, ann_GPP_C , PR, Class, River, Type1, Type2, nwis_gage, Disch_cv:Stream_PAR_sum)
...
autositeslong2<-litautositeslong %>% filter(year>0) %>% 
                        mutate(Type1 = case_when(primary_class == "Heterotrophic"~"Heterotrophic",
                                                 primary_class == "urban"~"Urban",
                                                 primary_class == "agri" ~"Agricultural",
                                                 primary_class == "desert" ~ "Desert",
                                                 primary_class == "snowmelt" ~ "Snowmelt"))
                          #%>% select(site_name, year, ann_GPP_C, PR, Class, axis1, axis2, labels)


rda_litauto <-  ggplot(data=autositeslong2 %>%  mutate(Class = case_when(PR<1~"Heterotrophic",
                                                                         PR>1~"Autotrophic")) %>%
                         rename(`Annual Trophic State` = Class),
                       aes(x=axis1, y=axis2)) + 
  geom_vline(xintercept = c(0), color = "grey70", linetype = 2) +
  geom_hline(yintercept = c(0), color = "grey70", linetype = 2) +  
  scale_x_continuous(sec.axis = dup_axis(labels=NULL, name=NULL)) +
  scale_y_continuous(sec.axis = dup_axis(labels=NULL, name=NULL)) +    
  geom_mark_ellipse(aes(fill = `Annual Trophic State`, label = `Annual Trophic State`, linetype = `Annual Trophic State`, alpha  = `Annual Trophic State` ),
                    expand=0, alpha =.1, show.legend = FALSE) +
  scale_fill_manual(values = c("green4","tan2"))+
  
  
  geom_point(data= autositeslong2 %>% mutate(Class = case_when(PR<1~"Heterotrophic", PR>1~"Autotrophic")) %>% 
               rename(Classification = Type1, `Annual P:R` = PR), 
             aes(x=axis1, y=axis2, colour=`Annual P:R`, shape = Classification), 
  ...
  scale_colour_stepsn(colors = terrain.colors(10, rev=T))+
    geom_point(data=litsiteslong2, aes(x=axis1, y=axis2, shape = Type1, color = "Literature"), color="blue", fill = "blue",
               size=5, show.legend = F, alpha = .5) +
    geom_text_repel(data = litsiteslong2, 
              mapping = aes(x=axis1, y=axis2, label = River), 
              inherit.aes = FALSE, color="blue",force = 2, force_pull = 10, max.time = 4) +
        theme_bw()+theme(panel.grid = element_blank(), legend.direction = "vertical",legend.text.align =0, 
                   legend.position = c(.8,.15), legend.box.just = "right", legend.box = "horizontal")+
    labs(x = "pca1 (32.34%)", y = "pca2 (16.88%)")+coord_equal(ratio = 1, xlim = c(-2,2))+
  scale_shape_manual(values = c(18,25, 17, 1,8, 10, 15))

rda_litauto

cowplot::save_plot("Heterotrophy_Figure6_PCA.png", rda_litauto, dpi = 600, base_width = 8, base_height = 8)
